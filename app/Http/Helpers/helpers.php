...
{
    if($precision == null) $precision = BasicSettingsProvider::get()?->precision ?? 2;

    // dd($precision, number_format($amount, $precision, ".", ""));
    ...
    if($precision == "double") {
        ...
        // dd(number_format($amount, $precision, ".", ""));

        // $amount = ($precision == 0) ? number_format($amount, 0, ".", "") : number_format($amount, $precision, ".", "");
...
function remove_speacial_char($string, $replace = "") {
...
function generate_unique_reference_number() {
    return uniqid("rf-") . '_' . strtoupper(generate_random_string_number(5));
}
